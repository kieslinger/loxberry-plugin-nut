...
LoxBerry::Web::lbheader("NUT Plugin V$version", "http://www.loxwiki.eu/NUT/", "help.html");
...
#$template->param( WEBSTATUS => "http://$ENV{SERVER_NAME}:$server_port/plugins/$lbpplugindir/status.cgi");  
 ...
 if ($EMAIL == 1) {
	$template->param( EMAILSENDYES => "selected=selected");
	$template->param( EMAILSENDNO => "");
} else {
	$template->param( EMAILSENDYES => "");
	$template->param( EMAILSENDNO => "selected=selected");
}
...
LOGEND "NUT Admin Setting finish.";
